...
public class CubeRenderer
{
    ...
    private void BuildSphere(float radius, int segments)
    {
        ...
        for (int lat = 0; lat <= segments; lat++)
        {
            ...
            for (int lon = 0; lon <= segments; lon++)
            {
                ...
                _vertices.Add(x); _vertices.Add(y); _vertices.Add(z); _vertices.Add(u); _vertices.Add(v);
            }
        }
        ...
        for (int lat = 0; lat < segments; lat++)
        {
            for (int lon = 0; lon < segments; lon++)
            {
                ...
                _indices.Add((uint)first); _indices.Add((uint)second); _indices.Add((uint)(first + 1));
                _indices.Add((uint)second); _indices.Add((uint)(second + 1)); _indices.Add((uint)(first + 1));
            }
        }
        if (_vertexArray != 0) UploadGeometry();
    }
    ...
    private void BuildCylinder(float radius, float height, int segments)
    {
        ...
        for (int i = 0; i <= segments; i++)
        {
            ...
            _vertices.Add(x); _vertices.Add(halfHeight); _vertices.Add(z); _vertices.Add(u); _vertices.Add(1f);
            _vertices.Add(x); _vertices.Add(-halfHeight); _vertices.Add(z); _vertices.Add(u); _vertices.Add(0f);
        }
        ...
        for (int i = 0; i < segments; i++)
        {
            ...
            _indices.Add((uint)idx); _indices.Add((uint)(idx + 1)); _indices.Add((uint)(idx + 2));
            _indices.Add((uint)(idx + 1)); _indices.Add((uint)(idx + 3)); _indices.Add((uint)(idx + 2));
        }
        if (_vertexArray != 0) UploadGeometry();
    }
    ...
    public void LoadCustomModel(string filePath)
    {
        try
        {
            ...
            if (scene == null || scene.MeshCount == 0) throw new Exception("No meshes found");
            ...
            foreach (var mesh in scene.Meshes)
            {
                ...
                for (int i = 0; i < mesh.VertexCount; i++)
                {
                    ...
                    _customVertices.Add(v.X); _customVertices.Add(v.Y); _customVertices.Add(v.Z); _customVertices.Add(uv.X); _customVertices.Add(uv.Y);
                }
            ...
            {
                float x = _customVertices[i*5], y = _customVertices[i*5+1], z = _customVertices[i*5+2];
    ...
    public void Render()
    {
        ...
        if (indexCount > 0) GL.DrawElements(PrimitiveType.Triangles, indexCount, DrawElementsType.UnsignedInt, 0);
        GL.BindVertexArray(0);
    }
